...
class FdtdSimPlugin(Plugin):
    def __call__(self, gridx, gridy, detector, objects, sources, pml_width, total_time):
        ...
        try:
            ...
            for obj in objects:
                grid[
                    obj["xrange"][0] : obj["xrange"][1],
                    obj["yrange"][0] : obj["yrange"][1],
                    0,
                ] = fdtd.Object(permittivity=obj["permittivity"], name=obj["name"])

            # Add sources
            for src in sources:
                if src["type"] == "line":
                    grid[
                        src["xrange"][0] : src["xrange"][1],
                        src["yrange"][0] : src["yrange"][1],
                        0,
                    ] = fdtd.LineSource(
                        period=src["period"],
                    ...
                    raise ValueError(
                        "Source type not supported, please use 'line' or 'point'"
                    )

            # Add detector
            if detector["type"] == "line":
                grid[
                    detector["xrange"][0] : detector["xrange"][1],
                    detector["yrange"][0] : detector["yrange"][1],
                    0,
                ] = fdtd.LineDetector(name=detector["name"])
            elif detector["type"] == "current":
                grid[
                    detector["xrange"][0] : detector["xrange"][1],
                    detector["yrange"][0] : detector["yrange"][1],
                    0,
                ] = fdtd.CurrentDetector(name=detector["name"])
            elif detector["type"] == "block":
                grid[
                    detector["xrange"][0] : detector["xrange"][1],
                    detector["yrange"][0] : detector["yrange"][1],
                    0,
                ] = fdtd.BlockDetector(name=detector["name"])
            else:
                raise ValueError(
                    "Detector type not supported, please use 'line', 'current' or 'block'"
                )
            ...
            plt.title(
                "Detected E field absolute value summed over the whole detector over time"
            )
            plt.plot(
                np.sum(
                    np.linalg.norm(grid.detectors[0].detector_values()["E"], axis=2),
                    axis=1,
                )
            )

            plt.savefig("detected_e_field.png", bbox_inches="tight")
            imge = Image.open("detected_e_field.png")
            ...
            plt.xlabel("Time step")
            plt.title(
                "Detected H field absolute value summed over the whole detector over time"
            )
            plt.plot(
                np.sum(
                    np.linalg.norm(grid.detectors[0].detector_values()["H"], axis=2),
                    axis=1,
                )
            )

            plt.savefig("detected_h_field.png", bbox_inches="tight")
            imgh = Image.open("detected_h_field.png")
